...
pgfault(u_int va)
{
	...
	// u_int perm = ((Pte *)(& vpt))[PTX(va)] & 0xfff;
	// Wrong !!!!!!!!!!!!!!
	va = ROUNDDOWN(va, BY2PG);
	u_int perm = ((Pte *)(* vpt))[VPN(va)] & 0xfff;
	if(!(perm & PTE_COW)){
		user_panic("the page is not COW! ");
		return ;
	}
	...
	if((syscall_mem_alloc(0, tmp, perm & (PTE_V | PTE_R))<0)){
		user_panic("User pgfault alloc failed ! ");
	}
...
duppage(u_int envid, u_int pn)
{
	...
	if(syscall_mem_map(0, addr, envid, addr, perm) < 0){
		user_panic("syscall_mem_map for son in duppage error !");
	}
	if(syscall_mem_map(0, addr, 0, addr, perm) < 0){
		user_panic("syscall_mem_map for father in duppage error !");
	}
...
fork(void)
{
	...
	if(ret < 0){
		user_panic("fork syscall_set_pgfault_handler failed !");
	}
