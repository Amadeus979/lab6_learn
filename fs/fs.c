...
diskaddr(u_int blockno)
{
	if (super && blockno >= super->s_nblocks) {
		user_panic("blockno %08x is greater s_noblocks\n", blockno);
	}
	return DISKMAP + (blockno << 12);
	// DISKMAP + blockno << 12 is (DISKMAP + blockno) << 12
}
...
free_block(u_int blockno)
{
	...
	if(blockno <= 0){
		user_panic("free block number %08x check Wrong",blockno);
		return ;
	}
...
/*to see how ingenious it is :
	...
	int main(){
		int i = 0;
		printf("i\ti/32\ti%%32\t1<<(i%%32)\n");
		for(i = 0; i < 1024 ; i++){
	//		printf("%d\t%d\t%d\t%08x\n",i,i/32,i%32,1<<(i%32));
			printf("%d\t%d\t%d\t",i,i/32,i%32);	
			output_b(i);
		}
	}
	*/
