...
void init_disk() {
    ...
    /*  2 + i:  Block 0 : Boot Sector and Partition Table
                Block 1 : Super Block
                Block 2 ~ 2 + i: Free Block Bitmap(hold )           
    */
    for(i = 0; i < nbitblock; ++i) {
        memset(disk[2+i].data, 0xff, NBLOCK/8);
        // 8 is bit numbers of 0xff;
    }
    if(NBLOCK != nbitblock * BIT2BLK) {
        diff = NBLOCK % BY2BLK / 8;
        // NBLOCK % BY2BLK : the over part of a BY2BLK(size of Block 2)
