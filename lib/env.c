                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    for (i = 0; i < bin_size; i += BY2PG) {
        ...
        if(page_alloc(&p) < 0){
            printf("Load mapper wrong, page_alloc no mem!\n");
            return -E_NO_MEM;
        }
        if(i==0){
            bcopy(
        ...
        if(r < 0){
            //E_NO_MEM
            printf("Load mapper wrong, page_insert no mem!");
            return r;
        }
    }
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    if(env_alloc(&e,0)<0){
        printf("env_alloc in env_create_priority wrong--no mem!\n");
        return ;
    }
