...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
	...
	if((perm&PTE_COW)!=0||(perm&PTE_V)==0){
		printf("PTE_V == 0 || PTE_COW is allowed!\n");
		return -E_INVAL;
	}
	if(va>=UTOP||va<0){
		printf("va out of [0,UTOP) !\n");
		return -E_INVAL;
	}
	ret = envid2env(envid, &env, 1);
	if(ret<0){
		printf("envid2env(envid, &env, 0)<0 !\n");
		return ret;
	}
	ret = page_alloc(&ppage);
	if(ret<0){
		printf("page_alloc(&ppage)<0 !\n");
		return ret;
	}
	ret = page_insert(env->env_pgdir, ppage, va, perm);
	if(ret<0){
		printf("page_insert(env->env_pgdir, ppage, va, perm)<0 !\n");
		return ret;
	}
	return 0;
}
				...
				u_int perm)
{
	...
	if(srcva >= UTOP || srcva < 0 || dstva >= UTOP || dstva < 0){
		printf("srcva:%x and dstva:%x should in [0,%x)\n",srcva,dstva,UTOP);
		return -E_INVAL;
	}
	// if((perm&PTE_COW)!=0||(perm&PTE_V)==0){
	// 	printf("PTE_V == 0 || PTE_COW is allowed!\n");
	...
	// }
	if((perm & PTE_V)==0){
		printf("Sorry,in sys_mem_map perm is invalid.\n");
		return -E_INVAL;
	}
	if((ret = envid2env(srcid, &srcenv, 0)) < 0){
		printf("envid2env(srcid, &srcenv, 0)) < 0 !\n");
		return ret;
	}
	if((ret = envid2env(dstid, &dstenv, 0)) < 0){
		printf("envid2env(dstid, &dstenv, 0)) < 0 !\n");
		return ret;
	}
	if((ppage = page_lookup(srcenv->env_pgdir, round_srcva, &ppte)) == 0){
		printf("srcenv not exist page at %x.\n",round_srcva);
...
int sys_mem_unmap(int sysno, u_int envid, u_int va)
{
	...
	struct Env *env;
	if(va>=UTOP||va<0){
		printf("va:%x out of [0,UTOP) !\n",va);
		return -E_INVAL;
	}
	if((ret = envid2env(envid, &env, 0))<0){
		printf("envid2env(envid:%x, &env, 0))<0 !\n",envid);
		return ret;
	}
...
int sys_env_alloc(void)
{
	...
	if((r = env_alloc(&e, curenv->env_id))<0){
		printf("env_alloc(&e, curenv->env_id)<0 !\n");
		return r;
	}
					 ...
					 u_int perm)
{
	...
	if(srcva != 0){
		...
		int ret = sys_mem_map(sysno, curenv->env_id, srcva, e->env_id, 
...
int sys_write_dev(int sysno, u_int va, u_int dev, u_int len)
{
    ...
    int flag = ((dev + len <= 0x10000020) && (dev >= 0x10000000)) || ((dev + len <= 0x130004200) && (dev >= 0x13000000)) || ((dev + len <= 0x15000200) && (dev >= 0x15000000));
	if(!flag){
		return -E_INVAL;
	}
